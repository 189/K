    ...
    (function($) {
        ...
        var touchBinding = {
            ...
            tap: function(node, fn, ctx) {
                $.each(node, function(nd){
                	var startTx, startTy, startTime;
                	...
                	nd.addEventListener('touchend', function(e) {
            ...
            press: function(node, fn, ctx) {
                $.each(node, function(nd){
                	var startTx, startTy, pressTimer;
                	...
                	nd.addEventListener('touchmove', function(e) {
                	    var touches = e.touches[0],
                	        endTx = touches.clientX,
                	        endTy = touches.clientY;

                	    // 一旦触发touchmove 视为非 press事件 清除 pressTimer 计时 
                	    if (pressTimer || (Math.abs(endTx - startTx) > 10 && Math.abs(endTy - startTy) > 10)) {
                	...
                	nd.addEventListener('touchend', function(e) {
                		// touchend 之后若计时器还存在(未执行完毕) 视为非 press 事件 清除计时
                	    pressTimer && clearTimeout(pressTimer);
                	}, false);
                })
            },
            ...
            swipe: function(node, fn, ctx) {
                $.each(node, function(nd){
                	...
                	nd.addEventListener('touchend', function(e) {
                	    ...
                	    if (Math.abs(startTx - endTx) > 20 || Math.abs(startTy - endTy) > 20) {
                	        fn.call(ctx);
                	    }
                	}, false);
                })
            ...
            swipeUp: function(node, fn, ctx) {
                $.each(node, function(nd){
                	...
                	nd.addEventListener('touchend', function(e) {
                	    ...
                	    if (Math.abs(startTx - endTx) < Math.abs(startTy - endTy)) {
                	        if (startTy - endTy > 20) {
                	            fn.call(ctx);
                	        }
                	    }
        ...
        // 附加 touch 事件到 $.dom 原型链上
        $.each(touchBinding, function(binding, type){
        	$.fn[type] = function(fn){
        		binding(this, fn, this);
        	};
        });
